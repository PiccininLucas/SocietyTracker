...
test('sem sinal: inicia, fecha pelos dois gols, encadeia o próximo jogo e sincroniza ao voltar', async ({
  ...
  const all = ((await (await request.get('/api/sessions/' + round.id + '/matches')).json()) as Summary[])
    .map((m) => [m.sequence, m.homeTeamName, m.homeScore, m.awayScore, m.awayTeamName, m.status, m.endReason])
...
test('falha transitória do servidor é reenviada sozinha, sem tocar em nada', async ({
  ...
  await expect(pendingNote(page)).toHaveCount(0, { timeout: 15000 });
  expect(calls).toBe(2);
  const [match] = (await (await request.get('/api/sessions/' + round.id + '/matches')).json()) as Summary[];
