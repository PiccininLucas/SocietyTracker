...
import {
  historicalTotalsForPeriod,
  type HistoricalPlayerTotal,
} from '../src/core/domain/entities/HistoricalPlayerTotal';
...
const registered = [
  { id: 'p', name: 'Antigo', isActive: false },
  { id: 'q', name: 'Novo', isActive: true },
];
function match(date: string, goal = true): MatchSummary {
  return {
    ...
    finishedAt: date + 'T20:07:00Z',
    homePlayers: ['p', 'q'].map((id) => ({
      id,
      name: id,
      nickname: null,
      isCaptain: false,
...
test('totais até o corte não duplicam gols, assistências ou bola murcha; médias usam somente registros', async () => {
  const matches = [
    match('2026-08-27', false),
    match('2026-09-03'),
    match('2026-09-10'),
    match('2026-09-17', false),
  ];
  const rows = playerPerformance(matches, registered, historical);
  const p = rows.find((r) => r.playerId === 'p')!;
  assert.deepEqual(
    [p.goals, p.assists, p.contributions, p.bottomCount, p.played, p.recordedGoals],
    [42, 23, 65, 4, 4, 2]
  );
  ...
  assert.deepEqual(
    [dto.totalMatchesPlayed, dto.totalSessionsPlayed, dto.goalsPerMatch],
    [null, null, null]
  );
  assert.equal(dto.recordedGoalsPerMatch, 0.5);
  const repo = {
    getLeaderboard: async () => [dto],
    getLeaderboardByDateRange: async () => [dto],
...
test('snapshot entra apenas em períodos que o contêm por inteiro e mantém jogador sem jogos', () => {
  ...
  const only = performanceLeaderboard(
    playerPerformance([], registered, historical).find((p) => p.playerId === 'p')!
  );
...
test('importação SQL concilia 39 jogadores, preserva vínculos e rejeita conflitos sem gravações parciais', async () => {
  ...
  try {
    ...
    const migration = await readFile(
      'supabase/migrations/202609100001_historical_totals.sql',
      'utf8'
    );
    ...
    const original = (
      await db.query<{ id: string }>(
        "INSERT INTO players(name,nickname,is_active) VALUES ('João','Chitão',false) RETURNING id"
      )
    ).rows[0].id;
    await db.exec(sql);
    const sums = (
      await db.query(`SELECT count(*)::int players, sum(goals)::int goals, sum(assists)::int assists,
      sum(bottom_count)::int bottom FROM historical_player_totals`)
    ).rows[0];
    assert.deepEqual(sums, { players: 39, goals: 357, assists: 258, bottom: 16 });
    assert.equal(
      (
        await db.query<{ player_id: string }>(
          "SELECT player_id FROM historical_player_totals WHERE source_name='Chitao'"
        )
      ).rows[0].player_id,
      original
    );
    assert.equal(
      (
        await db.query<{ is_active: boolean }>('SELECT is_active FROM players WHERE id=$1', [
          original,
        ])
      ).rows[0].is_active,
      false
    ...
    await db.exec(sql);
    assert.equal(
      (await db.query<{ n: number }>('SELECT count(*)::int n FROM players')).rows[0].n,
      39
    );
    assert.equal(
      (await db.query<{ n: number }>('SELECT count(*)::int n FROM historical_player_totals'))
        .rows[0].n,
      39
    );
    await assert.rejects(
      db.exec(sql.replace("('Chitao',60,37,23,0)", "('Chitao',61,38,23,0)")),
      /histórico diferente/
    );
    await db.exec('ROLLBACK');
    assert.equal(
      (
        await db.query<{ goals: number }>(
          "SELECT goals FROM historical_player_totals WHERE source_name='Chitao'"
        )
      ).rows[0].goals,
      37
    );
    ...
    const privileges = (
      await db.query(`SELECT has_table_privilege('anon','historical_player_totals','INSERT') AS insert,
      has_table_privilege('authenticated','historical_player_totals','UPDATE') AS update,
      has_table_privilege('anon','historical_player_totals','SELECT') AS read`)
    ).rows[0];
