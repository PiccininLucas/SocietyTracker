-- 202609210005: distinguir "acabou o tempo" de "encerrada por outro motivo".
--
-- A partida NAO encerra sozinha aos 0:00 — sempre ha um ultimo lance, e quem encerra e o
-- mesario. Isso continua igual. O que faltava era o ROTULO: todo encerramento que nao
-- fosse pela regra dos 2 gols virava 'manual', entao 'time_limit' nunca era gravado e o
-- selo "Tempo Oficial" de MatchHistoryCard.astro nunca aparecia.
--
-- Agora, ao finalizar: 2 gols -> two_goals; cronometro ja zerado -> time_limit;
-- encerrada antes do tempo (abandono, lesao) -> manual.
--
-- Unica diferenca em relacao a 202609210002: a expressao de finish_reason.

-- 202609210002: aplicar o placar enviado junto com a finalizacao.
-- Antes, o bloco que aplica homeScore/awayScore era exclusivo de p_action='score';
-- um PATCH com placar novo E status=finished encerrava a partida com o placar antigo.
-- Unica diferenca em relacao a 202609210001: a condicao do ramo de placar.
...
INSERT INTO public.society_schema_versions VALUES ('202609210005') ON CONFLICT DO NOTHING;
